...
# Certifique-se de que os arquivos sped_parser.py, xml_parser.py, etc. estão na pasta app/fiscal/
...
def executar_analise_completa(
    ...
    try:
        logging.info(f"Análise iniciada pelo usuário: {username}.")
        
        # 1. Extração de dados
        logging.info("Iniciando extração do SPED...")
        df_sped, df_sped_itens, df_sped_analitico_combinado, df_sped_cte_d190 = extrair_dados_sped(caminho_sped)
        ...
        df_itens_global = df_xml_itens # Armazena itens na global
        ...
        # -------------------------------------------------------------------------
        # 2. Conciliação TOTAL DA NOTA (C100, C500, D500 vs XML NF-e)
        # -------------------------------------------------------------------------
        ...
        # Tratamento de Nulos e Zeros nos Totais
        ...
        # Verificação de Impostos (Cabeçalho)
        ...
        # PIS/COFINS Calculado (Cabeçalho)
        ...
        if not df_itens_final.empty:
            ...
            if not df_sped_itens.empty:
                ...
                # Garante tipo INT para merge correto
                try:
                ...
                # Preenche valores numéricos nulos com 0.0
            ...
            if not df_itens_final.empty and not df_recon.empty: 
                ...
                # Puxa dados do cabeçalho para o item
                recon_cols_to_merge = [
                    ...
                    'STATUS_IPI', 'IPI_TOTAL_XML', # IPI TOTAL só para referência, não para overwrite
                ...
                # Renomeia colunas de PIS/COFINS calculados no item
                ...
                # Prorrateio de impostos QUE NÃO TÊM NO C170 (ou que são totais)
                # NÃO INCLUÍMOS IPI AQUI -> IPI VEM DO C170 DIRETO (VLR_IPI_SPED_ITEM)
                ...
                df_itens_final.drop(columns=['TAXA_PROPORCAO_ITEM'], inplace=True, errors='ignore')
                df_itens_final.drop(columns=['PIS_CALC_TOTAL', 'COFINS_CALC_TOTAL'], inplace=True, errors='ignore')
        ...
        # 4. Preparação das Abas para o Relatório
        colunas_relatorio = [
        ...
        if not df_itens_final.empty:
            colunas_itens_xml = [
                ...
                'STATUS_ICMS_ST', 'ICMS_ST_XML', 'ICMS_ST_SPED', 
                'STATUS_FCP_ST', 'FCP_ST_XML', 'FCP_ST_SPED', 
                'STATUS_IPI', 'VLR_IPI_SPED_ITEM', # <--- AQUI: Valor Bruto do C170
            ...
            # Renomeia para o nome final no Excel
        ...
        if not df_itens_final.empty and caminho_regras_detalhadas:
            ...
            for col in regras_ncm_cols:
                if col in df_itens_final.columns:
                    colunas_aliquota_xml.append(col)
            ...
            rename_map = {
                'VLR_ICMS_TOTAL_ITEM': 'VLR_ICMS_SOMA_SN', 
                'VLR_ICMS': 'VLR_ICMS', 
                'pICMS_XML': 'Aliquota ICMS (XML)', 
                'PRODUTO': 'Produto (Regra)', 
                'REGIME_PIS_COFINS': 'Regime PIS/COFINS (Regra)', 
                'MVA ORIGINAL': 'MVA Original (Regra)'
            }
        ...
        # Lógica simples de CT-e (mantida, mas sujeita ao problema Chave/Número se não ajustada no Parser)
        ...
        if not df_xml_cte_totais.empty and not df_report_cte.empty and 'NUM_CTE_SPED' in df_report_cte.columns:
            ...
            numeric_cols_cte = ['VL_OPR_XML', 'VL_BC_ICMS_XML', 'VL_ICMS_XML', 'VL_OPR_SPED_SUM', 'VL_BC_ICMS_SPED_SUM', 'VL_ICMS_SPED_SUM']
            for col in numeric_cols_cte:
                if col not in df_cte_merge.columns: df_cte_merge[col] = 0.0
            df_cte_merge[numeric_cols_cte] = df_cte_merge[numeric_cols_cte].fillna(0)

            df_cte_merge['STATUS_VALOR'] = np.where(
                (df_cte_merge['VL_OPR_XML'] - df_cte_merge['VL_OPR_SPED_SUM']).abs() <= tolerancia_valor, 'OK', 'DIVERGENTE'
            )
            df_cte_merge['STATUS_BC_ICMS'] = np.where(
                (df_cte_merge['VL_BC_ICMS_XML'] - df_cte_merge['VL_BC_ICMS_SPED_SUM']).abs() <= tolerancia_valor, 'OK', 'DIVERGENTE'
            )
            df_cte_merge['STATUS_ICMS'] = np.where(
                (df_cte_merge['VL_ICMS_XML'] - df_cte_merge['VL_ICMS_SPED_SUM']).abs() <= tolerancia_valor, 'OK', 'DIVERGENTE'
            )
            ...
            for col in status_cols_to_add:
                if col not in df_cte_merge.columns:
                    if col == 'CHV_CTE': df_cte_merge[col] = ''
                    else: df_cte_merge[col] = '' if (col.endswith('_XML') or col.startswith('CST')) else 0.0 if col.startswith('VL_') else 'N/A'
            ...
            for col in ['STATUS_VALOR', 'STATUS_BC_ICMS', 'STATUS_ICMS', 'STATUS_CFOP']:
                df_report_cte[col] = df_report_cte[col].fillna('N/A')
            for col in ['VL_OPR_XML', 'VL_BC_ICMS_XML', 'VL_ICMS_XML']:
                df_report_cte[col] = df_report_cte[col].fillna(0.0)
            if 'CHV_CTE_y' in df_report_cte.columns: df_report_cte.rename(columns={'CHV_CTE_y': 'CHV_CTE_XML'}, inplace=True)
            if 'CHV_CTE_XML' not in df_report_cte.columns: df_report_cte['CHV_CTE_XML'] = ''
            for col in ['CFOP_XML', 'CST_XML', 'CHV_CTE_XML']:
                if col in df_report_cte.columns: df_report_cte[col] = df_report_cte[col].fillna('')

        elif df_report_cte.empty:
            pass
        else:
            # SPED D190 tem dados, mas XML vazio
